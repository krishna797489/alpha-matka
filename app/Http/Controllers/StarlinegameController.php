...
class StarlinegameController extends Controller
{

    public function starline(){
        return view('startline_game.index');
    }
...
public function get(Request $request)
 {
  //echo"<pre>";print_r($request->All());exit;
     return Starlinegames::where('id',$request->id)->first();
 }

public  function status(Request $request)
 {
   $user =Starlinegames::where('id',$request->id)->first();
   if ($user) {
     if ($user->status) {
       $user->status = 0;
     } else {
 ...
 public function edit(Request $request)
    {
      $validator = Validator::make($request->all(), [
        ...
        if ($validator->fails()) {
          ...
          $games =Starlinegames::where('id',$request->id)->first();
          $games->name = $request->name;
          $games->start_time = $request->start_time;
          $games->end_time = $request->end_time;
          $games->code = $request->code;
    ...
    {
        $starlineGame = Starlinegames::find($request->id);
...
public function Gamesrated(){
    ...
    return view('startline_game.games_rates',compact('typesgames'));
 }
 public function Gamesratedpost(Request $request){
    ...
    $games = Starline::where('id', $request->id)->first();
